...
from wildland.storage_sync.base import BaseSyncer, SyncState, SyncEvent, SyncStateEvent, \
    SyncConflictEvent, SyncConflict, SyncErrorEvent
...
class SyncJob:
    ...
    def status(self) -> str:
        ...
        """
        ret = f'{self.container_name} {self.state} {str(self.source)} ' \
            ...
            with self.lock:
                try:
                    ...
                    if isinstance(event, SyncStateEvent):
                        ...
                        job.conflicts.append(event.conflict)
                    ...
                    # notify the client
                    event.job_id = job_id
                    logger.debug('Sending event notification: %s', event)
                    job.control_handler.send_event(event.toJSON())
                except KeyError:  # nonexistent job, probably already removed
                    pass
